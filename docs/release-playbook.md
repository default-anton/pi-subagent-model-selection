...
This is the canonical publish procedure for `pi-subagent-model-selection`.

## Scope

- Release channel: **latest** only (stable tags `vX.Y.Z`).
- Publish target: npm package **`pi-subagent-model-selection`**.
...
6. **Post-release verification**
   - `npm view pi-subagent-model-selection version dist-tags --json`
   - `npm view` may lag due cache. Verify with registry data:
     ```bash
     curl -s https://registry.npmjs.org/pi-subagent-model-selection \
...
- If a bad version is published:
  1. Deprecate bad version:
     - `npm deprecate pi-subagent-model-selection@X.Y.Z "broken release; upgrade to >=X.Y.Z+1"`
